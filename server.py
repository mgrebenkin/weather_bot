...
import get_forecast
import exceptions
from util import iterable_format_str
...
async def do_daily_forecasting(user_message: types.Message):

    #aioschedule.every().day.at(DAILY_FORECAST_TIME).do(send_tomorrow_forecast, message=message)
    job = aioschedule.every(3).seconds.do(send_test_message, message=user_message)
    while user_message.from_user.username in subscribers_for_daily_forecast:
        await aioschedule.run_pending()
        await asyncio.sleep(TASK_LOOP_PERIOD)
    aioschedule.cancel_job(job)
    logger.info(f'''Корутина завершена для пользователя {user_message.from_user.username}\n
    Все задачи:\n{iterable_format_str(asyncio.all_tasks())}
    Все расписания:\n{iterable_format_str(aioschedule.jobs)}''')
...
async def start_daily_forecasting(message: types.Message):
    ...
    Все задачи:\n{iterable_format_str(asyncio.all_tasks())}
    Все расписания: {iterable_format_str(aioschedule.jobs)}''')
...
async def stop_daily_forecasting(message: types.Message):
    ...
    logger.info(f'''Пользователь {message.from_user.username} отписан.\n
    Список подписавшихся: {subscribers_for_daily_forecast}\n
    Все задачи:\n{iterable_format_str(asyncio.all_tasks())}
    Все расписания:\n{iterable_format_str(aioschedule.jobs)}''')
...
async def log_debug_info(message: types.Message):
    ...
    Все задачи: {iterable_format_str(asyncio.all_tasks())}\n
    Все расписания: {iterable_format_str(aioschedule.jobs)}''')
